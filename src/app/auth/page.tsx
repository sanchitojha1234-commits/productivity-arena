...
const getErrorMessage = (err: any): string => {
  ...
  if (!err) result = "Empty error received.";
  else if (typeof err === "string") result = err;
  else if (typeof err === "object") {
    if (err.message && typeof err.message === "string") result = err.message;
    else if (err.error_description && typeof err.error_description === "string") result = err.error_description;
    else if (err.error && typeof err.error === "string") result = err.error;
    else if (err.toString && err.toString() !== "[object Object]") result = err.toString();
    else result = JSON.stringify(err);
  } else {
    result = JSON.stringify(err);
  }
  
  if (result === "{}" || result === '"{}"' || !result || result.trim() === "") {
    return "Connection error: Supabase rejected the request. Please verify that 'Allow Signups' is ON in your Supabase Auth Providers, and that your SMTP keys are correct.";
  }
      ...
      try {
        const { error } = await supabase.auth.signInWithOAuth({
          ...
          options: {
            ...
            queryParams: {
              prompt: 'select_account' // Forces the Google "Choose an account" screen
            }
          }
        });
        if (error) throw error;
        // The page will redirect to the provider's login screen
      } catch (err: any) {
        setErrorMsg(getErrorMessage(err));
        setIsLoading(false);
      }
