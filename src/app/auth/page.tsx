  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        setErrorMsg(typeof err === "string" ? err : err.message || JSON.stringify(err) || "Failed to send verification code. Please check your credentials.");
      } finally {
        setIsLoading(false);
      }
    } else {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        setErrorMsg(typeof err === "string" ? err : err.message || JSON.stringify(err) || "Failed to send registration code.");
      } finally {
        setIsLoading(false);
      }
    } else {
  ...
  const handleVerify = async (e: React.FormEvent) => {
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        setErrorMsg(typeof err === "string" ? err : err.message || JSON.stringify(err) || "Invalid or expired verification code.");
      } finally {
        setIsLoading(false);
      }
    } else {
