...
export default function Battles() {
  const { battles, level, startBattle, progressBattle, triggerMockOpponentProgress, pet, petMultiplier, joinLiveMatchmakingQueue, userId } = useApp();
  ...
  // Matchmaking trigger
  const handleRandomMatchmake = async () => {
    setIsSearching(true);
    ...
    try {
      const matchResult = await joinLiveMatchmakingQueue(targetText);
      
      if (matchResult === "waiting") {
        setSearchStage("Searching matchmaking lobby for active users...");
        
        // Wait in queue for 6 seconds, if no one pairs, fall back to bot practice!
        setTimeout(() => {
          setSearchStage("No active players in lobby. Initializing practice bot...");
          ...
          setTimeout(() => {
            ...
            setSearchStage("Practice Bot Found! Ready to clash!");
            
            setTimeout(() => {
              if (supabase) {
                const myUid = userId || "guest_" + Math.random().toString(36).substring(2, 9);
                supabase.from("matchmaking_queue").delete().eq("user_id", myUid);
              }
      ...
      if (matchResult && matchResult !== "waiting") {
        setMatchedOpponent({
          name: matchResult.opponentName,
          level: matchResult.opponentLevel,
          avatar: matchResult.opponentAvatar,
          quote: "Connected with Real Player!"
        });
        setSearchStage("Real Player Matched! Initializing live battle stream...");
    ...
    // Default Fallback to Bot if Database is not configured
    setTimeout(() => {
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {activeBattles.map((b) => (
                  ...
                  >
                    ...
                    {/* Cheat-proof automatic progress indicator */}
                    <div className="mt-6 pt-4 border-t border-slate-900/60 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      <div className="px-3 py-1.5 rounded-lg bg-cyan-500/5 border border-cyan-500/10 text-[9px] font-semibold text-cyan-400/90 leading-normal max-w-xs">
                        ⚡ Progress your clash by completing Tasks (+25%), Habits (+15%), or spending time in Focus Mode.
                      </div>
                    </div>
                  </div>
                ))}
