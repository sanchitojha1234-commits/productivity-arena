...
export default function Dashboard() {
  ...
  const activeFocusDays = weeklyData.filter(d => d.focus > 0);
  const bestDay = weeklyData.reduce((best, curr) => curr.focus > best.focus ? curr : best, { day: "None", focus: 0, completed: 0 });
  const avgSession = activeFocusDays.length > 0 ? Math.round(activeFocusDays.reduce((sum, curr) => sum + curr.focus, 0) / activeFocusDays.length) : 0;
        ...
        <div className="lg:col-span-8 glass-card p-6 rounded-3xl flex flex-col justify-between min-h-[380px]">
          ...
          <div className="border-t border-slate-900 pt-5 mt-4 flex items-center justify-between text-xs text-slate-400 font-semibold">
            ...
            <div className="flex items-center gap-4">
              <span>Best focus day: <strong className="text-slate-200">{bestDay.day} ({bestDay.focus}m)</strong></span>
              <span>Avg Session: <strong className="text-slate-200">{avgSession}m</strong></span>
            </div>
          </div>
        </div>
