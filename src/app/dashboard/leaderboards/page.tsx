...
export default function Leaderboards() {
  const { username, level, xp } = useApp();
  const [scope, setScope] = useState<Scope>("global");
  const [timeline, setTimeline] = useState<Timeline>("weekly");

  // Mock users database
  const getLeaderboardData = (): LeaderboardUser[] => {
    // Modify based on scope/timeline slightly for realistic data shifts
    if (scope === "friends") {
      return [
        { rank: 1, name: "FocusDragon", level: 12, xp: 1250, title: "Deep Work Master", avatar: "🐉", badge: "🥇" },
        { rank: 2, name: "FlowStateFanatic", level: 9, xp: 980, title: "Disciplined Warrior", avatar: "🤖", badge: "🥈" },
        { rank: 3, name: username, level: level, xp: xp, title: "ProcrastinatorHero", avatar: "🥷", badge: "🥉", isMe: true },
        { rank: 4, name: "StudyNerd44", level: 4, xp: 450, title: "Rookie Builder", avatar: "🎒", badge: "🎗️" },
        { rank: 5, name: "SlackerNoMore", level: 3, xp: 320, title: "Rookie Builder", avatar: "🐢", badge: "🎗️" }
      ];
    }

    if (scope === "university") {
      return [
        { rank: 1, name: "MIT_Coder", level: 45, xp: 4500, title: "Productivity King", avatar: "💻", badge: "🥇" },
        { rank: 2, name: "StanfordBuilder", level: 38, xp: 3820, title: "Disciplined Warrior", avatar: "🌲", badge: "🥈" },
        { rank: 3, name: "HarvardFocus", level: 32, xp: 3290, title: "Disciplined Warrior", avatar: "🏛️", badge: "🥉" },
        { rank: 4, name: "CaltechMath", level: 28, xp: 2810, title: "Deep Work Master", avatar: "🧮", badge: "🎗️" },
        { rank: 5, name: "OxfordWarlock", level: 25, xp: 2540, title: "Deep Work Master", avatar: "🧙‍♂️", badge: "🎗️" },
        { rank: 14, name: username, level: level, xp: xp, title: "Rookie Builder", avatar: "🥷", badge: "🎗️", isMe: true }
      ];
    }
    ...
    return [
      ...
      { rank: 5, name: "MIT_Coder", level: 45, xp: 4500, title: "Deep Work Master", avatar: "💻", badge: "🎗️" },
      { rank: 6, name: "StanfordBuilder", level: 38, xp: 3820, title: "Disciplined Warrior", avatar: "🌲", badge: "🎗️" },
      { rank: 7, name: "FlowStateFanatic", level: 35, xp: 3510, title: "Disciplined Warrior", avatar: "🤖", badge: "🎗️" },
      { rank: 8, name: "OxfordWarlock", level: 25, xp: 2540, title: "Deep Work Master", avatar: "🧙‍♂️", badge: "🎗️" },
      { rank: 42, name: username, level: level, xp: xp, title: "Rookie Builder", avatar: "🥷", badge: "🎗️", isMe: true }
    ];
  };
  ...
  const podium = [
    users.find((u) => u.rank === 2), // 2nd on left
    users.find((u) => u.rank === 1), // 1st in center
    users.find((u) => u.rank === 3)  // 3rd on right
  ].filter(Boolean) as LeaderboardUser[];

  const runnersUp = users.filter((u) => u.rank > 3);
              ...
              {runnersUp.map((u) => (
                ...
                >
                  <td className="py-4 pl-4 font-black">
                    {u.rank === 4 ? "#04" : u.rank === 5 ? "#05" : u.rank === 6 ? "#06" : u.rank === 7 ? "#07" : `#${u.rank}`}
                  </td>
