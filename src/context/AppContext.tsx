...
interface AppContextType {
  ...
  battles: Battle[];
  pet: Pet;
  statsHistory: Record<string, DailyStats>;
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [statsHistory, setStatsHistory] = useState<Record<string, DailyStats>>({});
  ...
  useEffect(() => {
    ...
    const savedStatsHistory = localStorage.getItem("pa_stats_history");
    if (savedStatsHistory) {
      setStatsHistory(JSON.parse(savedStatsHistory));
    }
  ...
  const addFocusMinutes = (minutes: number) => {
    ...
    setStatsHistory((prev) => {
      const todayStats = prev[todayStr] || { focus: 0, completed: 0 };
      const updated = {
        ...prev,
        [todayStr]: { ...todayStats, focus: todayStats.focus + minutes }
      };
      saveToStorage("pa_stats_history", updated);
      updateProfileInDb({ stats_history: updated });
      return updated;
    });
        ...
        if (t.id === id) {
          ...
          if (status === "Completed" && t.status !== "Completed") {
            ...
            setStatsHistory((prevStats) => {
              const todayStats = prevStats[todayStr] || { focus: 0, completed: 0 };
              const updatedStats = {
                ...prevStats,
                [todayStr]: { ...todayStats, completed: todayStats.completed + 1 }
              };
              saveToStorage("pa_stats_history", updatedStats);
              updateProfileInDb({ stats_history: updatedStats });
              return updatedStats;
            });
